...
use MichaelSchrenk\Bundle\Library\Http as MS_Http;
use MichaelSchrenk\Bundle\Library\Parse as MS_Parse;
use MichaelSchrenk\Bundle\Library\ResolveAddresses as MS_ReResolveAddresses;
...
class DownloadImages
{
    ...
    /**
     * Construct func that used by DownloadImages
     */
    public function __constrcut()
    {
        $this->http = new MS_Http();
        $this->parse = new MS_Parse();
        $this->resolveAddresses = new MS_ReResolveAddresses();
    }
    ...
    ***********************************************************************/
    public function download_images_for_page($target)
    {
        ...
        $web_page = $this->http->http_get($target, $referer="");
        ...
        $page_base = $this->resolveAddresses->get_base_page_address($target);
        ...
        $img_tag_array = $this->parse->parse_array($web_page['FILE'], "<img", ">");
        ...
        for ($xx=0; $xx<count($img_tag_array); $xx++) {
            $image_path = $this->parse->get_attribute($img_tag_array[$xx],  $attribute="src");
            echo " image: ".$image_path;
            $image_url = $this->resolveAddresses->resolve_address($image_path, $page_base);
            if ($this->resolveAddresses->get_base_domain_address($page_base) == $this->resolveAddresses->get_base_domain_address($image_url)) {
                ...
                $this_image_file =  $this->download_binary_file($image_url, $ref="");
